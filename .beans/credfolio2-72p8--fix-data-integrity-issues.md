...
- [x] Visual verification via `@qa` subagent (via Task tool, for UI changes) - N/A (backend only)
- [ ] ADR written via `/decision` skill (if new dependencies, patterns, or architectural changes were introduced) - N/A (no new patterns)
