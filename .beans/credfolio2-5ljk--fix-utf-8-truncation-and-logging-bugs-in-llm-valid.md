...
## Acceptance Criteria
- [x] UTF-8-safe truncation function implemented and tested
- [x] All string truncation uses UTF-8-safe function
- [x] Logging captures original size before truncation
- [x] Tests verify multi-byte character handling
...
- [ ] Visual verification via `@qa` subagent (via Task tool, for UI changes) - N/A: backend-only changes
- [ ] ADR written via `/decision` skill (if new dependencies, patterns, or architectural changes were introduced) - N/A: bug fix, no new patterns
